 ...
 *    (a Memory Trail is for handoff context, not a live AI session)
 */
...
- This Memory Trail is intended for safe AI handoff only.
...
function buildMarkdown(project: ProjectMemory, generatedAt: string): string {
  ...
  lines.push(`# Memory Trail: ${sanitize(project.name)}`);
  lines.push('');
...
function buildClaude(project: ProjectMemory, generatedAt: string): string {
  ...
  lines.push(`    - This Memory Trail is for safe AI handoff only.`);
  lines.push(`  </safe_export_notes>`);
  lines.push('');
...
/**
 * Generates a Memory Trail for the given project.
...
/**
 * Generates a Memory Trail text for a user-defined custom platform.
 *
