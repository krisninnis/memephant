 ...
 *    (a Context Passport is for handoff context, not a live AI session)
 */
...
import {
  formatGameContextCodex,
  formatGameContextMarkdown,
  formatGameContextXml,
} from './gameContextFormatter';

function workflowModeMarkdown(project: ProjectMemory): string | null {
  const mode = getWorkflowModeConfigForProject(project.workflowMode, project);
  if (!mode) return null;
...
function pushGameContextMarkdownSection(lines: string[], project: ProjectMemory): void {
  const block = formatGameContextMarkdown(project, (value) => clean(value, project));
  if (!block) return;
  lines.push(block);
  lines.push('');
}
...
- This Context Passport is intended for safe AI handoff only.
...
function buildMarkdown(project: ProjectMemory, generatedAt: string): string {
  ...
  pushGameContextMarkdownSection(lines, project);
...
function buildClaude(project: ProjectMemory, generatedAt: string): string {
  ...
  const gameContext = formatGameContextXml(project, (value) => clean(value, project));
  if (gameContext) {
  ...
  lines.push(`    - Local folder paths have been excluded.`);
    lines.push(`    - This Context Passport is for safe AI handoff only.`);
  lines.push(`  </safe_export_notes>`);
  lines.push('');
...
function buildCodex(project: ProjectMemory, generatedAt: string): string {
  ...
  const gameContext = formatGameContextCodex(project, (value) => clean(value, project));
  if (gameContext) {
    lines.push(gameContext);
    lines.push('');
  }
...
function buildCustomXmlLike(project: ProjectMemory, platform: CustomPlatform): string {
  ...
  lines.push(`<memory_trail platform="${sanitize(platform.name)}">`);
  lines.push(`  <project>${sanitize(project.name)}</project>`);
  lines.push('');
